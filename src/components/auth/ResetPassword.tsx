      ...
      >
        ...
        <div className="md:col-span-5 relative hidden md:flex flex-col justify-between p-10 bg-slate-50/50 border-r border-slate-100 overflow-hidden">
          ...
          <div className="flex items-center gap-2 text-[#DA4A1B]">
            ...
            <span className="text-[10px] font-mono uppercase tracking-[0.25em] font-black">Gatekeeper Auth v1.0</span>
          </div>
          ...
          <div className="space-y-2 text-left">
            <h4 className="text-slate-800 font-mono text-xs uppercase tracking-wider font-semibold">Join the Engine Matrix</h4>
            <p className="text-slate-400 text-xs leading-relaxed max-w-[220px]">
              Deploy performant applications, access secure nodes, and construct system pipelines effortlessly.
            </p>
          </div>
        </div>
        ...
        <div className="md:col-span-7 flex flex-col justify-center p-8 sm:p-12 lg:p-16 relative z-10 bg-white">
          ...
          {/* Back to Authentication Entry Hyperlink */}
          <div className="text-left mb-6">
            <Link
              to="/login"
              className="inline-flex items-center gap-2 text-xs font-semibold text-slate-400 hover:text-slate-800 transition-colors duration-200"
            >
              <ArrowLeft size={14} />
              <span>Login</span>
            </Link>
          ...
          <div className="text-left space-y-2.5 mb-10">
            <h1 className="text-3xl sm:text-4xl font-semibold tracking-tight text-slate-900">
              Reset <span className="font-mono uppercase font-black text-[#DA4A1B]">Password</span>
            </h1>
            <p className="text-slate-500 text-xs sm:text-sm">
              Input your authorized routing gateway below to deploy password recovery parameters.
            </p>
          </div>

          {/* Form Action Handler Core */}
          {!isSubmitted ? (
            <form onSubmit={handleSubmit} className="space-y-6 text-left">

              {/* Input Element Field: Email */}
              <div className="space-y-2">
                <label htmlFor="email" className="text-[11px] font-mono uppercase tracking-wider text-slate-400 font-bold block pl-1">
                  Secure Routing Gateway Address
                </label>
              ...
              <motion.button
                ...
                type="submit"
                disabled={isLoading}
                className="w-full relative mt-2 bg-[#DA4A1B] text-white py-3.5 px-4 rounded-xl font-bold text-sm tracking-wide uppercase transition-all duration-300 flex items-center justify-center gap-2 overflow-hidden shadow-[0_6px_20px_rgba(218,74,27,0.15)] disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isLoading ? (
                  <>
            ...
            /* Succession State Notification Feedback */
            <motion.div
              initial={{ opacity: 0, scale: 0.95 }}
              animate={{ opacity: 1, scale: 1 }}
              className="text-left bg-emerald-500/5 border border-emerald-500/10 rounded-2xl p-6 space-y-3"
            >
              <div className="flex items-center gap-2 text-emerald-600 font-bold text-sm uppercase tracking-wide font-mono">
                <ShieldCheck size={18} />
                <span>Transmission Dispatched</span>
              </div>
              <p className="text-slate-600 text-sm leading-relaxed">
                An internal verification pipeline link has been routed to <strong className="text-slate-800">{email}</strong>. Check your terminal logs or inbox to proceed.
              </p>
            </motion.div>
          )}
